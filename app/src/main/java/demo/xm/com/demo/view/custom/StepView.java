...
public class StepView extends View {
    ...
    public StepView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < a.getIndexCount(); i++) {
            attr = a.getIndex(i);
            switch (attr) {
                case R.styleable.StepView_arcSize:
                    mCircleSize = a.getInteger(attr, 30);
                    break;

                case R.styleable.StepView_arcTextSize:
                    mCircleTextSize = a.getInteger(attr, 30);
                    break;

                case R.styleable.StepView_bgArcColor:
                    mBgArcColor = a.getColor(attr, Color.GRAY);
                    break;

                case R.styleable.StepView_progressArcColor:
                    mProgressArcColor = a.getColor(attr, Color.RED);
                    mStepColor = mProgressArcColor;
                    break;

                case R.styleable.StepView_arcProgress:
