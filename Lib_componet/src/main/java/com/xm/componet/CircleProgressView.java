...
public class CircleProgressView extends View {
    ...
    public CircleProgressView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < a.getIndexCount(); i++) {
            ...
            if (attr == R.styleable.CircleProgressView_circleSize) {
                mCircleWidth = a.getInteger(attr, 30);

            } else if (attr == R.styleable.CircleProgressView_originalColor) {
                mOriginalColor = a.getColor(attr, Color.GRAY);

            } else if (attr == R.styleable.CircleProgressView_progressColor) {
                mProgressColor = a.getColor(attr, Color.RED);

            } else if (attr == R.styleable.CircleProgressView_progress) {
